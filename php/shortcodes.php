...
*/
function wpcookielaw_delete_cookies_shortcode( $atts ) {
	extract( shortcode_atts( array(
		'text' => 'Delete Cookies'
	), $atts ) );
	return "<a href='' id='wpcookielaw-cookie-delete'>{$text}</a>";
}
...
*/
function wpcookielaw_table_shortcode( $atts ) {
	
	/** RICHARDASHBY EDIT: only add CSS if table is being used */
	wp_enqueue_style( 'wpcookielaw-table-style' );
	/** END EDIT */
	...
	$ret = '<table class="wpcookielaw-' . $style . '"><thead><tr>';
	$ret .= '<th class="wpcookielaw-column-1">Cookie</th>';
	$ret .= '<th class="wpcookielaw-column-2">Type</th>';
	$ret .= '<th class="wpcookielaw-column-3">Duration</th>';
	$ret .= '<th class="wpcookielaw-column-4">Description</th></tr>';
	$ret .= '</thead><tbody>';
	
	if ( !$cookies->have_posts() ) {
		$ret .= '<tr class="wpcookielaw-row"><td colspan="2" class="wpcookielaw-column-empty">' . $not_shown_message . '</td></tr>';
	}
	...
	while ( $cookies->have_posts() ) : $cookies->the_post();
		...
		// Output HTML:
		$ret .= '<tr class="wpcookielaw-row"><td class="wpcookielaw-column-1">' . get_the_title() . '</td>';
		$ret .= '<td class="wpcookielaw-column-2">' . $cookie_type .'</td>';
		$ret .= '<td class="wpcookielaw-column-3">' . $cookie_duration .'</td>';
		$ret .= '<td class="wpcookielaw-column-4">' . get_the_content() .'</td>';
		$ret .= '</tr>';
	endwhile;
	$ret .= '</tbody></table>';
	return $ret;
...
function wpcookielaw_shortcode_accept_button( $atts ) {
	extract( shortcode_atts( array(
		'colour' => 'green'
	), $atts ) );
	...
	$settings = wp_parse_args( wpcookielaw_get_admin_settings(), $defaults );
...
function wpcookielaw_shortcode_more_link( $atts ) {
	return wpcookielaw_shortcode_button_DRY_code( 'button_2' );
}


/** Returns HTML for a generic button */
function wpcookielaw_shortcode_main_button( $atts ) {
	$defaults = array(
		'button_1_text' => 'Accept',
		'button_1_url' => '#',
	...
	$link_tag = '<a href="' . $url . '" id="' . wpcookielaw_remove_hash ( $settings['button_1_action'] ) . '" ';
...
/** Returns HTML for a generic button */
function wpcookielaw_shortcode_button_DRY_code( $name ) {
	$arr = wpcookielaw_get_admin_settings();
	$settings = array();
	$class_name = '';
	...
	$link_tag = '<a href="' . $url . '" id="' . wpcookielaw_remove_hash ( $settings['button_x_action'] ) . '" ';
