 ...
 */
function wpcookielaw_get_json_settings() {
	...
	$slim_settings = array(
		...
		'button_1_button_hover'			=> (wpcookielaw_su_hex_shift( $settings['button_1_button_colour'], 'down', 20 )),
		...
		'button_2_button_hover'			=> (wpcookielaw_su_hex_shift( $settings['button_2_button_colour'], 'down', 20 )),
...
*/
function wpcookielaw_inject_cli_script() {
	...
	if ( $the_options['is_on'] == true ) {
		...
		$notify_html = '<div id="' . wpcookielaw_remove_hash( $the_options["notify_div_id"] ) . '"><span>' . $str . '</span></div>';
		
		if ( $the_options['showagain_tab'] === true ) {
			$notify_html .= '<div id="' . wpcookielaw_remove_hash( $the_options["showagain_div_id"] ) . '"><span id="cookie_hdr_showagain">' . $the_options["showagain_text"] . '</span></div>';
		}
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function() {
				cli_show_cookiebar({
					settings: '<?php echo wpcookielaw_get_json_settings(); ?>'
				});
			});
			//]]>
		</script>
...
*/
function wpcookielaw_enqueue_frontend_scripts() {
	$the_options = wpcookielaw_get_admin_settings();
	if ( $the_options['is_on'] == true ) {
		...
		wp_register_style( 'wpcookielaw-style', CLI_PLUGIN_URL . 'css/cli-style.css', null, $version );
		wp_enqueue_style( 'wpcookielaw-style' );
		
		wp_enqueue_script( 'wp-cookie-law-script', CLI_PLUGIN_URL . 'js/wpcookielaw.js', array( 'jquery' ), $version );
	}
	wp_register_style( 'wpcookielaw-table-style', CLI_PLUGIN_URL . 'css/cli-tables.css', null, $version );
}
 ...
 */
function wpcookielaw_su_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
...
function wpcookielaw_remove_hash( $str ) {
	if ( $str{0} == "#" ) {
	...
	return wpcookielaw_remove_hash( $str );
}
 ...
 */
function wpcookielaw_make_hex_colour_6_chars( $hex ) {
	$str = wpcookielaw_remove_hash( $hex );
	if ( strlen( $str ) == 3 ) {
...
function wpcookielaw_debug_alertbox( $gubbins ) {
...
function wpcookielaw_debug_echo( $gubbins ) {
	if ( ! CLI_PLUGIN_DEVELOPMENT_MODE )
		return;
...
function wpcookielaw_debug_admin_settings( $break ) {
	if ( ! CLI_PLUGIN_DEVELOPMENT_MODE )
		return;
	$settings = wpcookielaw_get_admin_settings();
	$ret = '<p>Settings: ';
