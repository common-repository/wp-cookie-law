 ...
 */
function wpcookielaw_get_default_settings() {
	$settings_v0_9 = array(
		...
		'notify_div_id' 				=> '#wp-cookie-law-bar',
		...
		'showagain_div_id' 				=> '#wp-cookie-law-again',
		'showagain_tab' 				=> true,
 ...
 */
function wpcookielaw_delete_settings() {
...
*/
function wpcookielaw_colourpicker_enabled() {
	$settings = wpcookielaw_get_admin_settings();
...
function wpcookielaw_get_admin_settings() {
	$settings = wpcookielaw_get_default_settings();
	$stored_options = get_option( CLI_SETTINGS_FIELD );
	if ( !empty( $stored_options ) ) {
		foreach ( $stored_options as $key => $option ) {
			$settings[$key] = wpcookielaw_sanitise( $key, $option );
		}
	}
...
function wpcookielaw_copy_old_settings_to_new() {
	$new_settings = wpcookielaw_get_admin_settings();
	$old_settings = get_option( CLI_ADMIN_OPTIONS_NAME );
...
function wpcookielaw_migrate_to_new_version() {
 ...
 */
function wpcookielaw_allowed_html() {
	$allowed_html = array(
 ...
 */
function wpcookielaw_allowed_protocols() {
 ...
 */
function wpcookielaw_sanitise($key, $value) {
	$ret = null;
	...
	switch ($key) {
		...
		case 'notify_message':
			$ret = wp_kses( $value, wpcookielaw_allowed_html(), wpcookielaw_allowed_protocols() );
			break;
		// URLs only:
		case 'button_1_url':
		case 'button_2_url':
 ...
 */
function wpcookielaw_register_custom_menu_page() {
	add_submenu_page(
		'edit.php?post_type=wpcookielaw',
		'Cookie Law Settings',
		'Cookie Law Settings',
		'manage_options',
		'wp-cookie-law',
		'wpcookielaw_print_admin_page'
	);
}


/**
 Registers dashboard scripts and styles used for WP Cookie Law plugin settings panel
 Important: these scripts only load on the plugin settings page (avoids conflicts)
 Hooked into admin_enqueue_script
 */
function wpcookielaw_custom_dashboard_styles( $hook ) {
	if ( 'wpcookielaw_page_wp-cookie-law' != $hook )
        return;
    
	wp_register_style( 'wpcookielaw-admin-style', plugins_url('/wp-cookie-law/css/cli-admin-style.css') );
    wp_enqueue_style( 'wpcookielaw-admin-style' );
}


/**
 Adds custom script to wpcookielaw admin panel to run the colour pickers
 Important: these scripts only load on the plugin settings page (avoids conflicts)
 Hooked into admin_footer
 */
function wpcookielaw_custom_dashboard_styles_my_colours() {
	$screen = get_current_screen();
	if ( $screen->post_type != 'wpcookielaw' ) {
		return;
	}
}
 ...
 */
function wpcookielaw_get_js_actions() {
	$js_actions = array(
 ...
 */
function wpcookielaw_get_fonts() {
	$fonts = Array(
 ...
 */
function wpcookielaw_get_button_sizes() {
	$sizes = Array(
		'Extra Large'	=> 'super',
		'Large'			=> 'large',
		'Medium'		=> 'medium',
 ...
 */
function wpcookielaw_print_combobox_options( $options, $selected ) {
