...
function wpcookielaw_print_admin_page() {
	...
	// Get options:
	$the_options = wpcookielaw_get_admin_settings();
	
	// Check if form has been set:
	if ( isset( $_POST['update_admin_settings_form'] ) ) {
		// Check nonce:
		check_admin_referer( 'wpcookielaw-update-' . CLI_SETTINGS_FIELD );
		foreach ( $the_options as $key => $value ) {
			if (isset($_POST[$key . '_field'])) {
				// Store sanitised values only:
				$the_options[$key] = wpcookielaw_sanitise($key, $_POST[$key . '_field']);
			}
		}
		...
		// Check nonce:
		check_admin_referer( 'wpcookielaw-update-' . CLI_SETTINGS_FIELD );
		wpcookielaw_delete_settings();
		$the_options = wpcookielaw_get_admin_settings();
	}
	else if ( isset ( $_POST['revert_to_previous_settings'] ) ) {
		if ( !wpcookielaw_copy_old_settings_to_new() ) {
	...
	if ( isset ( $_POST['cli-migration-button'] ) ) {
		if ( isset( $_POST['cli-migration_field'] ) ) {
			switch ( $_POST['cli-migration_field'] ) {
				case '2':
					// Migrate but keep
					if ( !wpcookielaw_migrate_to_new_version() ) {
						echo '<h3>ERROR MIGRATING SETTINGS (ERROR: 2)</h3>';
					}
					break;
				case '3':
					// Just use this version
					wpcookielaw_update_to_latest_version_number();
					break;
				default:
					// Form error, ignore
	...
	if ( function_exists('wp_nonce_field') ) 
		wp_nonce_field('wpcookielaw-update-' . CLI_SETTINGS_FIELD);
		...
		<div class="cli-plugin-left-col width-62">
			<div class="pad-10">
					...
					<div class="left">
						...
						<?php
							...
							if ( $the_options['is_on'] == true ) {
								...
								$span_tag .= 'Your WP Cookie Law bar is switched on</span>';
							}
							else {
								$img_tag .= 'cross.png" alt="cross icon" />';
								$span_tag .= 'Your WP Cookie Law bar is switched off</span>';
							}
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								...
								<?php wpcookielaw_print_combobox_options( wpcookielaw_get_fonts(), $the_options['font_family'] ) ?>
								</select>
							</td>
						</tr>
					</table>
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								...
								<?php wpcookielaw_print_combobox_options( wpcookielaw_get_js_actions(), $the_options['button_1_action'] ) ?>
								</select>
							</td>
						</tr>
						...
						<tr valign="top">
							...
							<td>
								...
								<?php wpcookielaw_print_combobox_options( wpcookielaw_get_button_sizes(), $the_options['button_1_button_size'] ); ?>
								</select>
							</td>
						</tr>
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								...
								<?php wpcookielaw_print_combobox_options( wpcookielaw_get_button_sizes(), $the_options['button_2_button_size'] ); ?>
								</select>
							</td>
						</tr>
					</table><!-- end custom button -->
				...
				<dd class="cli-help">
					...
					<p>You can enter the shortcodes in the "message" field of the WP Cookie Law bar. They add nicely formatted buttons and/or links into the cookie bar, without you having to add any HTML.</p>
					...
					<pre>[cookie_audit]</pre><span>This prints out a nice table of cookies, in line with the guidance given by the ICO. <em>You need to enter the cookies your website uses via the WP Cookie Law menu in your WordPress dashboard.</em></span>
					
					<pre>[delete_cookies]</pre><span>This shortcode will display a normal HTML link which when clicked, will delete the cookie set by WP Cookie Law (this cookie is used to remember that the cookie bar is closed).</span>
			...
			</dl><!-- end of wpcookielaw-accordion -->
	...
	<?php
		...
		//DEBUG:
		echo wpcookielaw_debug_admin_settings( true );
		echo wpcookielaw_debug_echo (wpcookielaw_get_json_settings() );
