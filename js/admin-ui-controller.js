jQuery(document).ready(function() {
	...
	*/
	...
	switch ( getURLParameter("cliopentab") ) {
		case "0":
			jQuery("#wpcookielaw-accordion").accordion( { autoHeight: false, active: 0 } );
			break;
		case "2":
			jQuery("#wpcookielaw-accordion").accordion( { autoHeight: false, active: 2 } );
			break;
		case "3":
			jQuery("#wpcookielaw-accordion").accordion( { autoHeight: false, active: 3 } );
			break;
		case "1":
		default:
			jQuery("#wpcookielaw-accordion").accordion( { autoHeight: false, active: 1 } );
	}
	...
	jQuery('#is_on_field_yes').click(function(){
		// Cookie bar is on
		f.text("Your WP Cookie Law bar is switched on");
		t.text("Your WP Cookie Law bar is switched on");
		t.removeClass('warning');
		swap_icon( cookie_bar_on_icon );
	});
	jQuery('#is_on_field_no').click(function(){
		// Cookie bar is off
		f.text("Your WP Cookie Law bar is switched off");
		t.text("Your WP Cookie Law bar is switched off");
		t.addClass('warning');
